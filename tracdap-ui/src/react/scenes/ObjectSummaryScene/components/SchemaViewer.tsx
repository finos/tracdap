/**
 * A component that shows a summary of a particular version of a schema stored in TRAC. This is used by both
 * the ObjectSummaryScene and the SchemaInfoModal. This component is for viewing in a browser, there is a sister
 * component called {@link SchemaViewerPdf} that is for viewing in a PDF. These two components need to be kept
...
import {ButtonToDownloadSchema} from "../../../components/ButtonToDownloadSchema";

/**
 * An interface for the props of the SchemaViewer component.
 */
export interface Props {
    ...
    /**
     * The tag selector for the schema to show a summary of.
     */
...
export const SchemaViewer = (props: Props) => {
    ...
    useEffect(() => {
        ...
        if (getTagFromUrl && !props.tagSelector && objectVersion === undefined && tagVersion === undefined) {
            ...
            navigate(`${ObjectSummaryPaths[trac.metadata.ObjectType.SCHEMA].to}/${objectId}/latest/latest`)
            ...
            navigate(`${ObjectSummaryPaths[trac.metadata.ObjectType.SCHEMA].to}/${props.tagSelector.objectId}/${props.tagSelector.latestObject ? "latest" : props.tagSelector.objectVersion}/${props.tagSelector.latestTag ? "latest" : props.tagSelector.tagVersion}`)
        }
    ...
    const urlDetails = !getTagFromUrl ? undefined : getTagSelectorFromUrlParameters({objectType: trac.ObjectType.SCHEMA, objectId, objectVersion, tagVersion, searchAsOf})
    ...
    if (urlDetails !== undefined && status === "failed") {
        urlDetails.errorMessages.push("That schema object ID and/or version numbers do not exist.")
    }
    ...
    useEffect(() => {
        ...
        if (!isDownloading && tag && tag.header?.objectId === objectId && objectVersion === "latest") {
        ...
        <React.Fragment>
            ...
            {!props.tagSelector && urlDetails?.errorMessages && urlDetails?.errorMessages.length > 0 &&
                <Alert className={"mt-4 mb-4"} variant={"danger"} showBullets={true} listHasHeader={true}>
                    {["Downloading the schema information failed due to the URL parameters being invalid "].concat(urlDetails?.errorMessages ?? [])}
                </Alert>
            }
            ...
            {!isDownloading && tag && (props.tagSelector || urlDetails?.errorMessages.length === 0) &&
                <React.Fragment>
                    ...
                    <TextBlock>
                        The buttons below can be used to download the JSON and PDF versions of the schema information shown
                        below.
                    </TextBlock>
                    ...
                    <ButtonToDownloadSchema className={"me-5"}
                                            loading={isDownloading}
                                            tag={tag}
                    />
                    ...
                    <SchemaFieldsTable className={"mt-5"} fields={tag.definition?.schema?.table?.fields}>
