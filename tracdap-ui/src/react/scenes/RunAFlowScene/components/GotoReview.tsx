...
import {reviewJob, setValidationChecked} from "../store/runAFlowStore";
import {useAppDispatch, useAppSelector} from "../../../../types/types_hooks";

export const GotoReview = () => {

    // Get what we need from the store
    const canRun = useAppSelector(state => state["runAFlowStore"].validation.canRun)
    const {isValid: isValidSettings} = useAppSelector(state => state["runAFlowStore"].validation)
    ...
    const handleReview = (): void => {
        ...
        const invalidModels = objectsContainsValue(isValidSettings.models, false)
        ...
        if (invalidModels || invalidParameters || invalidInputs) {

            const messageString = commasAndAnds([invalidModels ? "models" : undefined, invalidParameters ? "parameters" : undefined, invalidInputs ? "input datasets" : undefined].filter(isDefined))
