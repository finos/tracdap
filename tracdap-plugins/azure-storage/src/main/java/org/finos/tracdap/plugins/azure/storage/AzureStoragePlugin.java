...
public class AzureStoragePlugin extends TracPlugin {

    private static final String PLUGIN_NAME = "AZURE_STORAGE";
    ...
    private static final List<PluginServiceInfo> serviceInfo = List.of(
            new PluginServiceInfo(IFileStorage.class, AZURE_BLOB_STORAGE, List.of("blob")));
    ...
    protected <T> T createService(String service, Properties properties, ConfigManager configManager) {
        ...
        if (service.equals(AZURE_BLOB_STORAGE)) {
            var instance = properties.getProperty(IStorageManager.PROP_STORAGE_KEY);
            return (T) new AzureBlobStorage(instance, properties);
        }
