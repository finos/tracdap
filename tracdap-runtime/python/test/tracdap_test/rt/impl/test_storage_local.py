...
# ----------------------------------------------------------------------------------------------------------------------
# DATA STORAGE - Using SQL with SQLite
# ----------------------------------------------------------------------------------------------------------------------


class SqliteDataStorageTest(unittest.TestCase, DataStorageTestSuite):

    storage_root: tempfile.TemporaryDirectory
    db_file: pathlib.Path

    # Storage format is not used for SQL backends
    storage_format = ""
    ...
    def make_storage(cls):
        ...
        cls.storage_root = tempfile.TemporaryDirectory()
        cls.db_file = pathlib.Path(cls.storage_root.name).joinpath("sqlite_storage.db")

        bucket_config = _cfg.PluginConfig(
            protocol="SQL",
            properties={
                "dialect": "sqlite",
                "sqlite.database": str(cls.db_file),
